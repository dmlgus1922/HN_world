...
def photo_take_func():
    ...
    return (st.image(st.session_state.photo, channels='BGR'),
    st.button(
        '등록', 
...
def user_regis():
    ...
    with photo_regis_area:
        if st.button(
            ...
            picture, btn = photo_take_func()
            CAPTURED_PHOTO.image(picture)
            btn
    ...
    with cam_area:
        WEB_CAM = st.image([])
        show_cam(WEB_CAM) # 영상 표출 때문에 무한루프 중
